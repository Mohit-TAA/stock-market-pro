...
- Streamlit Cloud compatible (pandas 2.0+, no kaleido, exponential backoff)
"""
...
class ProfessionalDataManager:
    ...
    def _fetch_with_retry(self, ticker, period, interval, max_retries=5, base_delay=2):
        """Fetch data with exponential backoff retries to handle rate limiting."""
...
class ProfessionalMarketPlatform:
        ...
        with st.sidebar:
            ...
            with st.form("symbol_form"):
                ...
                st.session_state.analysis_period = st.selectbox(
                    ...
                    index=Config.PERIOD_OPTIONS.index(st.session_state.analysis_period) if st.session_state.analysis_period in Config.PERIOD_OPTIONS else 12
                )
    ...
    def render_stock_analysis(self):
        ...
        def safe_return(col_name, days):
            ...
            # Fallback: compute return over approximate days using date lookup
            if len(data) > 0:
        ...
        # Performance bar chart only (no heatmap)
        ...
        # Fundamental groups with enhanced styling
        ...
        # Fundamental Charts (using cached data manager)
    ...
    def _display_fundamental_charts_enhanced(self, symbol):
        # Use data manager for cached access
        ...
        with col1:
            if st.button("Generate PDF Report", use_container_width=True, type="primary"):
                with st.spinner("Generating comprehensive PDF report..."):
                    # Use selected period (original used "2y" – we use the sidebar period)
                    ...
                    # Generate technical charts for report with fallback to matplotlib if kaleido fails
                    charts = {}
                    if not data.empty:
                        for chart_name, indicator in [("RSI", "RSI"), ("MACD", "MACD"), ("Bollinger Bands", "Bollinger Bands")]:
                            try:
                                ...
                                # Try kaleido first, fallback to matplotlib
                                try:
                                    img_bytes = fig.to_image(format="png", width=800, height=400, engine='kaleido')
                                except Exception:
                                    img_bytes = fig.to_image(format="png", width=800, height=400, engine='matplotlib')
                                charts[chart_name] = img_bytes
                            except Exception as e:
                                st.warning(f"Could not generate chart for {chart_name}: {e}")
        ...
        with col2:
            if st.button("Generate HTML Report", use_container_width=True):
                with st.spinner("Generating HTML report..."):
                    ...
                    # Generate technical charts as base64 for HTML embedding (with fallback)
                    charts = {}
                    if not data.empty:
                        for chart_name, indicator in [("RSI", "RSI"), ("MACD", "MACD"), ("Bollinger Bands", "Bollinger Bands")]:
                            try:
